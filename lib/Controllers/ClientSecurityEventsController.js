...
class ClientSecurityEventsController {
     ...
     * @param {int} input['perPage'] The number of entries per page returned. Acceptable range is 3
     * - 1000.
    ...
    static getNetworkClientSecurityEvents(input, callback) {
        ...
        if (input.networkId === null || input.networkId === undefined) {
            ...
            _callback(_err, null, null);
            return Promise.reject(_err);
        } else if (input.perPage === null || input.perPage === undefined) {
            const _err = { errorMessage: 'The property `perPage` in the input object cannot be null.',
                errorCode: -1 };
            _callback(_err, null, null);
            return Promise.reject(_err);
        }
